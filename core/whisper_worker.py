...
這是 core 內部的執行單元。
用法: python -m core.whisper_worker <wav_path> <json_path>
"""
import os
import sys
import json
...
# 專案根目錄加入 path (此檔案在 core/whisper_worker.py)
...
def main():
    if len(sys.argv) != 3:
        print("Usage: whisper_worker <wav_path> <json_path>", file=sys.stderr)
        sys.exit(1)
    wav_path = sys.argv[1]
...
if __name__ == "__main__":
    try:
        ...
        print(f"whisper_worker failed: {e}", file=sys.stderr)
        sys.exit(1)
