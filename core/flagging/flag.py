...
class SentenceHealth(BaseModel):
    ...
    suggested_correction: Optional[str] = Field(
        None,
        description=(
            "The corrected text IF it is an ASR error. "
            "If it is just weird speech behavior (echolalia), leave this null."
        ),
    )
...
def analyze_batch_safe(batch_sentences: List[dict]) -> Optional[HealthReport]:
    ...
    for attempt in range(max_retries):
        try:
            ...
            print(
                f"      ⚠️ Timeout (Attempt {attempt+1}/{max_retries}). Retrying...",
                end="\r",
                flush=True,
            )
            time.sleep(2)
            ...
            for item in batch:
                ...
                if sid in assessment_map:
                    ...
                    if assessment.is_suspicious:
                        ...
                        item["review_reason"] = (
                            f"[{assessment.issue_category}] {assessment.reason}"
                        )
                        item["suggested_correction"] = assessment.suggested_correction
        else:
            print(" Failed. Skipping flags.", flush=True)
            for item in batch:
