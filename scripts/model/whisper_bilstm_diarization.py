...
依賴：`core/speaker_bilstm/`（已自 Neuro-AI 推論路徑萃取最小子集），並安裝 transformers、librosa 等（見 core/requirements.txt）。

環境變數（可選）：
  SPEAKER_INFER_BATCH_SIZE — 推論 DataLoader batch（預設 1，長 chunk 建議 1）
  SPEAKER_BILSTM_USE_MEDIAN — 是否對 frame 預測做中值濾波（預設 1）
"""
...
class _NeuroInferConfig:
    """不經過 Neuro-AI Config.__post_init__（避免訓練路徑驗證），只帶推論所需欄位。"""
